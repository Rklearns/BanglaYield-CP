...
The repository also includes supporting visual summaries such as:

- [plots/model_comparison.png](plots/model_comparison.png)
- [plots/mean_harvest.png](plots/mean_harvest.png)
- [plots/year_yield.png](plots/year_yield.png)
- [plots/shap_top15.png](plots/shap_top15.png)

These plots capture both the exploratory trends in the aggregated data and the final modeling behavior.
...
- `yieldML.ipynb` contains the complete workflow: loading, merging, preprocessing, model benchmarking, and explainability.
- `plots/` stores exported visualizations from exploratory analysis and post-modeling interpretation.
- `Results/` stores tabular outputs such as model metrics and prediction files.
- `requirements.txt` lists the Python dependencies needed to reproduce the notebook locally.
...
If `pip` has trouble reading `requirements.txt`, first note that the file is currently encoded in UTF-16 and may need conversion depending on your local environment.
...
The workflow produces two main classes of outputs:

### Results

- [Results/model_results.csv](Results/model_results.csv)
- [Results/model_predictions.csv](Results/model_predictions.csv)
...
Depending on the exact notebook run and export cells, additional explainability figures such as feature-importance or SHAP-specific charts may also be generated.
